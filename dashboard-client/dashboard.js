...
const MQTT_BROKER_URL = "mqtt://192.168.1.251:1884"; // Vi du: neu broker chay tren cung may o cong 1884
// *********************************************************
const MQTT_TOPIC_SUBSCRIBE = "/server/w_gateway";

// --- 1. HTTP Server de phuc vu file index.html ---
const server = http.createServer((req, res) => {
  if (req.url === "/" || req.url === "/index3.html") {
    fs.readFile(path.join(__dirname, "index3.html"), (err, data) => {
      if (err) {
        console.error("Loi doc file index.html:", err);
        res.writeHead(500);
